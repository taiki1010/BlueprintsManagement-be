...
class ArchitecturalDrawingRequestTest {
    ...
    public void before() {
        ...
        MockMultipartFile mockImage = new MockMultipartFile("imageFile", "image.png", "image/png",
                dummyImageBytes);
        architecturalDrawingRequest = new ArchitecturalDrawingRequest(blueprintId, createdAt,
                mockImage);
    }
    ...
    class ValidBlueprintIdTest {
        ...
        void 図面idがUUID形式に適している場合正常に処理が実行されること(String blueprintId) {
            ...
            Set<ConstraintViolation<ArchitecturalDrawingRequest>> violations = validator.validate(
                    architecturalDrawingRequest);
        ...
        void 図面idがUUID形式に適していない場合バリデーションチェックされること(
                String blueprintId) {
            architecturalDrawingRequest.setBlueprintId(blueprintId);
            Set<ConstraintViolation<ArchitecturalDrawingRequest>> violations = validator.validate(
                    architecturalDrawingRequest);
